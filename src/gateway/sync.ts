...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  // Syncs: clawdbot config, skills, and agent workspaces (SOUL.md, memory, etc.)
  const syncCmd = `rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ && rsync -r --no-times --delete /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ && mkdir -p /root/clawd/agents && rsync -r --no-times --delete /root/clawd/agents/ ${R2_MOUNT_PATH}/agents/ && date -Iseconds > ${R2_MOUNT_PATH}/.last-sync`;
